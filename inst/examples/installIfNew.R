...
doc <- "Usage: installIfNew.R [-r REPO...] [-l LIBLOC] [-h] [-x] [-d DEPS] [--error] [--] [PACKAGES ...]
...
Examples:
  installIfNew.R -l /tmp/lib Rcpp BH                         # install into given library
  installIfNew.R -- --with-keep.source drat                  # keep the source
  installIfNew.R -- --data-compress=bzip2 stringdist         # prefer bz2 compression

installIfNew.R is part of littler which brings 'r' to the command-line.
...
install_packages2 <- function(pkgs, ..., error = FALSE) {
  ...
  if(length(pkgs) == 0) {
    warning("All specified packages are already installed.")
  } else {
